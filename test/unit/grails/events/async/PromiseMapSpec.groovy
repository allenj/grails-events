...
class PromiseMapSpec extends Specification {
	...
	void "Test PromiseMap with mixture of normal entries and promises populated via constructor"() {
		when:
			"A promise map is used with an onComplete handler"
		...
		then:
			"An appropriately populated map is returned to the onComplete event"
			result != null
			result["one"] == 1
			result["four"] == 4
			result["eight"] == 8
	...
	void "Test PromiseMap with then chaining"() {
		when:
			"A promise map is used with then chaining"
			def map = new PromiseMap<String, Integer>()
			map["one"] = { 1 }
