...
class PromiseSpec extends Specification {
	...
	void "Test promise timeout handling"() {
		when:
			"a promise that takes a while is created"
	...
	void "Test promise list handling"() {
		...
		when:
			"A promise list is created from two closures"
	...
	void "Test promise onComplete handling"() {
		...
		when:
			"A promise is executed with an onComplete handler"
		...
		then:
			"The onComplete handler is invoked and the onError handler is ignored"
			result == 2
			hasError == false
